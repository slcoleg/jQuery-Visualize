...
$.fn.visualize = function(options, container){
  return $(this).each(function(){
      ...
      var tableData = {
        dataGroups: function(){
          ...
          if(o.parseDirection == 'x'){
            self.find('tr:gt(0)').filter('tr').each(function(i){
              dataGroups[i] = {};
              dataGroups[i].points = [];
              dataGroups[i].color = colors[i];
              if(textColors[i]){ dataGroups[i].textColor = textColors[i]; }
              $(this).find('td').filter('td').each(function(){
                dataGroups[i].points.push( parseFloat($(this).text()) );
              });
            });
          }
          else {
            var cols = self.find('tr:eq(1) td').filter('td').size();
            for(var i=0; i<cols; i++){
              dataGroups[i] = {};
              dataGroups[i].points = [];
              dataGroups[i].color = colors[i];
              if(textColors[i]){ dataGroups[i].textColor = textColors[i]; }
              self.find('tr:gt(0)').filter('tr').each(function(){
                dataGroups[i].points.push( $(this).find('td').filter('td').eq(i).text()*1 );
              });
            };
          }
          return dataGroups;
        ...
        xLabels: function(){
          var xLabels = [];
          if(o.parseDirection == 'x'){
            self.find('tr:eq(0) th').filter('th').each(function(){
              xLabels.push($(this).html());
            });
          }
          else {
            self.find('tr:gt(0) th').filter('th').each(function(){
              xLabels.push($(this).html());
            });
          }
          return xLabels;
    ...
    if(o.appendKey){
      ...
      var selector;
      if(o.parseDirection == 'x'){
        selector = self.find('tr:gt(0) th').filter('th');
      }
      else{
        selector = self.find('tr:eq(0) th').filter('th');
      }
